...
# Setup logging
log_dir = os.path.join("quant_log", "tinyllama_quant_25", datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
...
def quantize_model_weights(model, bits=4, group_size=128):
    ...
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear):
            ...
            count += 1
    logger.info(f"    Quantized {count} Linear layers to INT{bits} (group_size={group_size})")
    return model
...
# ============================================================
logger.info("Loading pruned model + LoRA...")
...
logger.info(f"    Estimated INT4 model size: {est_size_int4:.1f} MB")
...
logger.info(f"    PPL: {ppl_int4['wikitext2']:.2f}, Time: {time_int4:.1f}s")
...
logger.info(f"    Estimated INT8 model size: {est_size_int8:.1f} MB")
...
logger.info(f"    PPL: {ppl_int8['wikitext2']:.2f}, Time: {time_int8:.1f}s")
...
logger.info("=" * 70)
logger.info(f"{'Model Version':<35} {'PPL':>8} {'Size(MB)':>10} {'Time':>10}")
logger.info("-" * 70)
logger.info(f"{'Original TinyLlama (FP16)':<35} {'17.42':>8} {'~2200':>10} {'--':>10}")
logger.info(f"{'Pruned 25%':<35} {'35.95':>8} {'~1762':>10} {'--':>10}")
logger.info(f"{'Pruned 25% + LoRA (FP16)':<35} {'26.71':>8} {'~1762':>10} {'--':>10}")
logger.info(f"{'Pruned + LoRA + W8 (per-group)':<35} {ppl_int8['wikitext2']:>8.2f} {f'~{est_size_int8:.0f}':>10} {f'{time_int8:.0f}s':>10}")
logger.info(f"{'Pruned + LoRA + W4 (per-group)':<35} {ppl_int4['wikitext2']:>8.2f} {f'~{est_size_int4:.0f}':>10} {f'{time_int4:.0f}s':>10}")
logger.info(f"{'Pruned + LoRA + Dynamic INT8':<35} {'81.82':>8} {'~250':>10} {'589s':>10}")
logger.info("=" * 70)
logger.info("\nNote: W4/W8 = weight-only quantization (similar to GPTQ/AWQ)")
logger.info("      Dynamic INT8 = PyTorch dynamic quantization (activations also quantized)")
