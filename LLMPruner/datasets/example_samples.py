...
def get_bookcorpus(tokenizer, n_samples, seq_len):
    try:
        ...
        print(f"[Warning] Failed to load bookcorpus: {e}")
        print("[Info] Using random token sequences for importance estimation instead.")
        vocab_size = tokenizer.vocab_size if hasattr(tokenizer, 'vocab_size') else 32000
        return torch.randint(1, vocab_size, (n_samples, seq_len))
